...
export function EditVictims(props: IPropsForm) {
  ...
  const handleUpadateUser = async (data: Yup.InferType<typeof schema>) => {
    try {
      await updateVictims(props.idVictim, data);
  ...
  const fetchVictimData = () => {
    findById(props.idVictim)
      .then((response) => {
        if (response && response.data) {
          ...
          setValue(
            ...
            response.data.rua_beco_travessa_estrada_ramal
          );
          ...
          setValue(
            ...
            response.data.presencafilhofamiliar
          );
      ...
      .catch((error: any) => {
        toast.error("Erro ao buscar os dados do usuário.", error?.response.data.detail);
      });
  };
